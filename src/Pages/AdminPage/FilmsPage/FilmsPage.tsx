...
function Films() {
  ...
  const arrFilm = useAppSelector(selectFilmArrFilm, _.isEqual);
  const dispatch = useAppDispatch();

  const handleDeleteFilm = useCallback(
    (maPhim: number, dispatch: AppDispatch) => {
      Swal.fire({
        title: 'Are you sure?',
  ...
  const renderColumns = useMemo(() => {
        ...
        render: (text, record, index) => {
          return (
            <Fragment>
              ...
              <NavLink
                className='text-xl p-2'
                to={`${process.env.REACT_APP_LINK_ADMIN_SHOWTIME_FILMS}/${record.maPhim}/${record.tenPhim}`}
              >
    ...
    return columns;
  }, [dispatch, handleDeleteFilm]);

  useEffect(() => {
    if (_.isEmpty(arrFilm)) {
      dispatch(getFilmAsync());
    }
  }, [arrFilm, dispatch, getFilmAsync]);
